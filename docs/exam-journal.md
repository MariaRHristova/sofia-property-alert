...
### 2026-06-23 - Vercel deployment prep and runtime hardening

- **Outcome:** Prepared the repository for a Vercel deployment by adding a `vercel.json` service config for `app/main.py`, making the runtime Vercel-aware for SQLite, email previews, cookie security, and public base URLs, and adding a regression test for the new configuration behavior. The live deployment is still pending Vercel authentication in this shell.
- **Approach and reasoning:** Chose the Services-style Vercel layout because the project is a single FastAPI app with templates and static assets rather than a separate frontend/backend monorepo. Kept local defaults unchanged and only switched to preview-safe filesystem paths when Vercel environment variables are present. Included the fixture HTML and Jinja/static assets in the deployment package so the app can render and parse deterministically on Vercel.
- **AI-assisted workflow:** Codex edited `app/config.py`, `app/main.py`, `vercel.json`, and `tests/test_config.py`, then ran the full automated suite. I also bootstrapped the Vercel CLI with `npx vercel --version`, which installed `vercel@54.15.0`, and attempted `npx vercel whoami` plus `npx vercel login`; both commands stalled waiting for account authentication.
- **AI tool choice:** Codex was used for the repository changes and verification because it could inspect the local workspace, patch the runtime behavior, and run the tests. The Vercel CLI was used for the deployment attempt because it is the supported command-line deployment path.
- **Key prompts:** "Ok, do it"
- **Validation:** Ran `powershell -ExecutionPolicy Bypass -File .\scripts\run_pytest_clean.ps1 -q` and got `24 passed, 1 warning in 6.49s`. Ran `.\.venv\Scripts\python -m ruff check .` and got `All checks passed!`. `npx vercel --version` reported `54.15.0`. `npx vercel whoami` and `npx vercel login` did not complete because the shell has no Vercel account credentials.
- **Challenges and learning:** `vercel.json` initially picked up a Windows BOM and had to be rewritten as UTF-8 without BOM so the CLI could parse it. The deployment cannot be completed from this shell without an authenticated Vercel session or token, so the live URL remains `TODO`.
- **Evidence:** `app/config.py`, `app/main.py`, `vercel.json`, `tests/test_config.py`, `docs/exam-journal.md`, `npx vercel --version`, the full test run, and `TODO: live Vercel deployment URL pending authentication`.